...
// ===== DURATION CACHE =====
const DURATION_CACHE_KEY = "yt_duration_cache_v1";

function getDurationCache() {
  try { return JSON.parse(localStorage.getItem(DURATION_CACHE_KEY) || "{}"); }
  catch { return {}; }
}
function saveDurationCache(cache) {
  try { localStorage.setItem(DURATION_CACHE_KEY, JSON.stringify(cache)); }
  catch {}
}
...
// قائمة خوادم Invidious للتبديل عند الفشل
...
async function fetchVideoDuration(id) {
  const cleanId = cleanVideoId(id);
  if (!cleanId) return 0;

  const cache = getDurationCache();
  if (cache[cleanId] !== undefined) return cache[cleanId];

  // تجربة خوادم Invidious بالتتابع
  for (const instance of INVIDIOUS_INSTANCES) {
    try {
      const res = await fetch(`${instance}/api/v1/videos/${cleanId}?fields=lengthSeconds`, {
        signal: AbortSignal.timeout(5000)
      });
      if (!res.ok) continue;
      const d = await res.json();
      const duration = d.lengthSeconds || 0;
      if (duration > 0) {
        const c = getDurationCache();
        c[cleanId] = duration;
        saveDurationCache(c);
        return duration;
      }
...
// ===== ENRICH DURATIONS — جلب مدة عدة فيديوهات بدفعات =====
async function enrichDurations(videos, onUpdate) {
  const cache = getDurationCache();
  const missing = videos.filter(v => cache[cleanVideoId(v.id)] === undefined);
  const BATCH = 4;
  ...
  for (let i = 0; i < missing.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async v => {
      ...
      if (onUpdate) onUpdate(v.id, duration);
    }));
  }
}
